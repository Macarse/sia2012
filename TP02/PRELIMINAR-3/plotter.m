function [  ] = plotter( g , g_deriv, N,input_gen ,epsilon, eta )
...
while (error > epsilon && debug < M) 
    ...
    fprintf('Epoca: %d\t Error:%g \tError minimo: %g\t media : %g \n ',debug, error, min_error, mean(errors));
end

hold on;
epocas = [0:1:debug];
semilogx(epocas, errors, 'r');
xlabel('Epocas');
