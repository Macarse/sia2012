...
\end{table}
	...
	Existe una excepci\'on en la remoci\'on de las fichas del tipo \textit{Season} o \textit{Flowers}. La excepci\'on esta basada en que cualquier par de fichas \textit{Season} o \textit{Flowers} puede ser removidas utilizando cualquier combinaci\'on entre las mismas, esto se debe a que, al existir solo una ficha con con el mismo tipo e identificador, no se pueden generar pares. \\
	Una ficha puede estar bloqueada en forma \textit{Horizontal}, \textit{Vertical} o \textit{Ambos}. Estos bloqueos se definen del siguiente modo:
	\begin{itemize}
...
\section*{Heur\'isticas}
	...
	Se considera \textit{Pair} a todo par de fichas que podr\'ian ser retiradas del tablero en un estado y \textit{Payers} a todo conjunto de todo estos \textit{Pares} con posibilidad de ser retirados en un estado del juego. \\
	...
	\begin{itemize}
		...
		donde $F_{1}$ y $F_{2}$ son las fichas que componen el \textit{Pair}. Esta heur\'istica puede ser mejorada para priorizar le remoci\'on de las fichas de niveles superiores agregando un modificador seg\'un el nivel en que se encuentre, por ejemplo:
			\begin{eqnarray}
				\textbf{h2} & = &( X_{1} + X_{2} ) * (N_{1} * C + N_{2} * C)
			\end{eqnarray}
				donde $N_{1}$ y $N_{2}$ son los niveles de las fichas $F_{1}$ y $F_{2}$ respectivamente, y $C$ hace referencia a una constante positiva la cual determina el grado de relevancia que se le quiera dar al nivel en que se encuentre la ficha.
...
Se puede observar a simple vista que para la estrategia \emph{DFS} se logran mejores resultados tanto para \emph{BFS} como para \emph{Profundizaci\'on iterativa}, en el sentido del tiempo promedio y nodos expandidos. Si bien todas las estrategias llegan a una soluci\'on en un tablero de nivel f\'acil, al agregar complejidad y cantidad de fichas (Por ejemplo, con 144 fichas) no se alcanza una soluci\'on. Esto podr\'ia deberse a que para el problema que se trata, siempre se necesita ir hasta el final del \'arbol de b\'usqueda, dado que se busca el camino que cubra todo el tablero y no m\'as de all\'i. Es por eso que si \emph{DFS} entra en la rama de la soluc\'ion, seguro llega a esta y no expande m\'as nodos. En cambio, \emph{BFS} no encontrar\'a la soluci\'on hasta explorar el \'ultimo nivel y lo mismo sucede con \emph{Profundizaci\'on iterativa}. Por estos motivos, las estrategias de b\'usqueda informadas son comparadas con \emph{DFS}. \\
